...
export class MockPersianProvider {
  async generate({ type, previous = [] }) { const choices = [['سه روش ساده برای شروع کار با هوش مصنوعی', 'از یک مسئله کوچک شروع کنید، خروجی را بررسی کنید و دستور خود را مرحله‌به‌مرحله بهتر کنید.'], ['اشتباه رایج: اعتماد کامل به اولین پاسخ هوش مصنوعی', 'پاسخ را با منبع معتبر مقایسه کنید، سؤال دقیق‌تر بپرسید و اطلاعات حساس را وارد نکنید.'], ['چطور یک ابزار هوش مصنوعی را درست ارزیابی کنیم؟', 'کیفیت خروجی، حریم خصوصی، زبان فارسی و هزینه واقعی را با یک نمونه کار ثابت بسنجید.'], ['یک پروژه واقعی: ساخت تقویم محتوایی با کمک هوش مصنوعی', 'هدف، مخاطب و چهار ستون محتوایی را مشخص کنید؛ سپس هر ایده را پیش از انتشار بازبینی کنید.']]; const [title, lesson] = choices.find(([candidate]) => !isDuplicateTopic(candidate, previous)) || choices[previous.length % choices.length]; const slides = [title, 'مسئله چیست؟', lesson, 'یک تمرین کوچک برای امروز', 'خروجی را انسانی بازبینی کنید', 'خلاصه و قدم بعدی']; return { title, hook: title, slides, script: `${title}. ${lesson} همین امروز این روش را روی یک کار کوچک امتحان کنید.`, caption: `${title}\n\n${lesson}\n\nاگر مفید بود ذخیره کنید و تجربه‌تان را بنویسید.`, hashtags: ['#هوش_مصنوعی', '#آموزش_هوش_مصنوعی', '#ابزار_هوش_مصنوعی', '#تولید_محتوا', '#mohammad_por_ai'], sources: [] }; }
}

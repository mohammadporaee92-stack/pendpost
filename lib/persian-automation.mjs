...
export class OllamaPersianProvider {
  ...
  async generate(context) {
    const schema = JSON.stringify(CONTENT_SCHEMA); const prompt = `فقط یک شیء JSON معتبر فارسی مطابق این JSON Schema بساز و هیچ متن یا markdown دیگری ننویس: ${schema}. آمار و منبع ساختگی ممنوع. نوع: ${context.type}. برند: ${JSON.stringify(context.profile)}. ممنوع: ${context.profile.forbiddenTopics.join('،')}. قبلی: ${JSON.stringify(context.previous.map((x) => x.title))}`;
    const response = await this.fetch(`${this.baseUrl}/api/generate`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: this.model, prompt, stream: false, think: false, format: CONTENT_SCHEMA, options: { temperature: 0 } }) });

...
export class JsonStore {
  ...
  withLock(name, operation) {
    fs.mkdirSync(this.root, { recursive: true }); const lock = `${this.file(name)}.lock`; const started = Date.now(); const token = crypto.randomBytes(32).toString('hex'); const owner = `owner-${String(started).padStart(13, '0')}-${token}`; const ownerPath = path.join(lock, owner); let markerCreated = false; let acquired = false;
    const pause = () => Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 10);
    const timedOut = () => Date.now() - started >= this.lockTimeoutMs;
    try {
      while (!acquired) {
        if (!markerCreated) {
          try { fs.mkdirSync(lock, { mode: 0o700 }); } catch (error) { if (error.code !== 'EEXIST') throw error; }
          try { fs.writeFileSync(ownerPath, `${process.pid}\n${new Date().toISOString()}\n`, { mode: 0o600, flag: 'wx' }); markerCreated = true; }
          catch (claimError) {
            if (!['ENOENT', 'ENOTDIR'].includes(claimError.code)) throw claimError;
            try {
              const legacy = fs.statSync(lock);
              if (legacy.isFile() && Date.now() - legacy.mtimeMs > this.staleLockMs) {
            ...
            if (timedOut()) throw new Error(`store_lock_timeout: ${name}`); pause(); continue;
          }
        }
        try { const now = new Date(); fs.utimesSync(ownerPath, now, now); }
        catch (touchError) { if (touchError.code === 'ENOENT') { markerCreated = false; continue; } throw touchError; }
        let owners;
        try { owners = fs.readdirSync(lock).filter((entry) => /^owner-\d{13}-[0-9a-f]{64}$/.test(entry)).sort(); }
        catch (readError) { if (readError.code === 'ENOENT' || readError.code === 'ENOTDIR') { markerCreated = false; continue; } throw readError; }
        if (!owners.includes(owner)) { markerCreated = false; continue; }
        const first = owners[0];
        if (first === owner) { acquired = true; break; }
        try {
          const firstPath = path.join(lock, first); const firstStat = fs.statSync(firstPath);
          if (Date.now() - firstStat.mtimeMs > this.staleLockMs) { try { fs.unlinkSync(firstPath); continue; } catch (unlinkError) { if (unlinkError.code !== 'ENOENT') throw unlinkError; } }
        } catch (statError) { if (statError.code === 'ENOENT') continue; throw statError; }
        if (timedOut()) throw new Error(`store_lock_timeout: ${name}`); pause();
      }
      return operation();
    } finally {
      if (markerCreated) try { fs.unlinkSync(ownerPath); } catch (error) { if (error.code !== 'ENOENT') throw error; }
    }
  }

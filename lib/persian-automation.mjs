...
export class JsonStore {
  ...
  withLock(name, operation) {
    ...
    while (!acquired) {
      try { fs.mkdirSync(lock, { mode: 0o700 }); fs.writeFileSync(path.join(lock, owner), `${process.pid}\n${new Date().toISOString()}\n`, { mode: 0o600 }); acquired = true; }
      catch (error) {
        if (error.code !== 'EEXIST') throw error;
        try {
          if (Date.now() - fs.statSync(lock).mtimeMs > this.staleLockMs) {
            const stale = `${lock}.stale-${crypto.randomUUID()}`;
            try { fs.renameSync(lock, stale); fs.rmSync(stale, { recursive: true, force: true }); continue; }
            catch (renameError) { if (renameError.code !== 'ENOENT' && renameError.code !== 'EEXIST' && renameError.code !== 'EPERM') throw renameError; }
          }
    ...
    try { return operation(); } finally {
      try { fs.unlinkSync(path.join(lock, owner)); } catch (error) { if (error.code !== 'ENOENT') throw error; }
      try { fs.rmdirSync(lock); } catch (error) { if (error.code !== 'ENOENT' && error.code !== 'ENOTEMPTY' && error.code !== 'EEXIST') throw error; }
    }
  }

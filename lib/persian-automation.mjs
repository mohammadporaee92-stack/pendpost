...
export class JsonStore {
  ...
  withLock(name, operation) {
    fs.mkdirSync(this.root, { recursive: true }); const lock = `${this.file(name)}.lock`; const started = Date.now(); let descriptor;
    while (descriptor === undefined) { try { descriptor = fs.openSync(lock, 'wx', 0o600); fs.writeFileSync(descriptor, `${process.pid}\n${new Date().toISOString()}\n`); } catch (error) { if (error.code !== 'EEXIST') throw error; try { if (Date.now() - fs.statSync(lock).mtimeMs > this.staleLockMs) { fs.unlinkSync(lock); continue; } } catch (statError) { if (statError.code === 'ENOENT') continue; throw statError; } if (Date.now() - started >= this.lockTimeoutMs) throw new Error(`store_lock_timeout: ${name}`); Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 10); } }
    try { return operation(); } finally { fs.closeSync(descriptor); try { fs.unlinkSync(lock); } catch (error) { if (error.code !== 'ENOENT') throw error; } }
  }

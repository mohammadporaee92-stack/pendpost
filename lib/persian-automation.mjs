  ...
  withLock(name, operation) {
    ...
    while (!acquired) {
      ...
      try {
        ...
        catch (publishError) { if (!['EEXIST', 'ENOTEMPTY', 'EPERM'].includes(publishError.code)) throw publishError; }
      } finally { fs.rmSync(candidate, { recursive: true, force: true }); }
      try {
        const inspected = fs.statSync(lock); const inspectedOwners = fs.readdirSync(lock).filter((entry) => entry.startsWith('owner-')).sort();
        if (Date.now() - inspected.mtimeMs > this.staleLockMs) {
          const stale = `${lock}.stale-${crypto.randomUUID()}`;
          try {
            fs.renameSync(lock, stale);
            const quarantined = fs.statSync(stale); const quarantinedOwners = fs.readdirSync(stale).filter((entry) => entry.startsWith('owner-')).sort();
            const sameGeneration = inspected.dev === quarantined.dev && inspected.ino === quarantined.ino && inspectedOwners.length === quarantinedOwners.length && inspectedOwners.every((entry, index) => entry === quarantinedOwners[index]);
            if (!sameGeneration) { fs.renameSync(stale, lock); continue; }
            fs.rmSync(stale, { recursive: true, force: true }); continue;
          } catch (renameError) { if (!['ENOENT', 'EEXIST', 'ENOTEMPTY', 'EPERM'].includes(renameError.code)) throw renameError; }
        }

...
export function contentSchemaFor(type) {
  const [minItems, maxItems] = type === 'carousel' ? [6, 8] : [5, 9];
  return { ...CONTENT_SCHEMA, properties: { ...CONTENT_SCHEMA.properties, slides: { ...CONTENT_SCHEMA.properties.slides, minItems, maxItems } } };
}
...
export class JsonStore {
  ...
  withLock(name, operation) {
    ...
    while (!acquired) {
      ...
      try {
        ...
        if (lockStat.isFile()) {
          ...
          if (!owners.length) {
            try { fs.rmdirSync(lock); continue; } catch (removeError) { if (!['ENOENT', 'ENOTEMPTY', 'EEXIST', 'EPERM'].includes(removeError.code)) throw removeError; }
          }
      ...
      } catch (statError) { if (!['ENOENT', 'ENOTDIR', 'EPERM'].includes(statError.code)) throw statError; }
    ...
    try { return operation(); } finally {
      ...
      if (removedOwnMarker) try { fs.rmdirSync(lock); } catch (error) { if (!['ENOENT', 'ENOTEMPTY', 'EEXIST', 'EPERM'].includes(error.code)) throw error; }
    }
  }
...
function parseOllamaJson(value) {
  if (typeof value !== 'string' || !value.trim()) throw new Error('ollama_invalid_json: empty_response');
  const trimmed = value.trim(); const candidates = [trimmed]; const fenced = trimmed.match(/^\`\`\`(?:json)?\s*([\s\S]*?)\s*\`\`\`$/i);
  if (fenced) candidates.push(fenced[1].trim());
  const firstBrace = trimmed.indexOf('{'); const lastBrace = trimmed.lastIndexOf('}'); if (firstBrace >= 0 && lastBrace > firstBrace) candidates.push(trimmed.slice(firstBrace, lastBrace + 1));
  for (const candidate of [...new Set(candidates)]) { try { return JSON.parse(candidate); } catch {} }
  throw new Error('ollama_invalid_json: response_not_parseable');
}
...
export class OllamaPersianProvider {
  ...
  async generate(context) {
    const responseSchema = contentSchemaFor(context.type); const schema = JSON.stringify(responseSchema); const [minSlides, maxSlides] = context.type === 'carousel' ? [6, 8] : [5, 9]; const retry = context.attempt ? `این تلاش شماره ${context.attempt + 1} است؛ پاسخ قبلی رد شد. ` : '';
    const prompt = `${retry}فقط یک شیء JSON معتبر فارسی مطابق این JSON Schema بساز و هیچ متن یا markdown دیگری ننویس: ${schema}. آرایه slides باید حداقل ${minSlides} و حداکثر ${maxSlides} رشتهٔ غیرخالی داشته باشد. آمار و منبع ساختگی ممنوع. نوع: ${context.type}. برند: ${JSON.stringify(context.profile)}. ممنوع: ${context.profile.forbiddenTopics.join('،')}. قبلی: ${JSON.stringify(context.previous.map((x) => x.title))}`;
    const response = await this.fetch(`${this.baseUrl}/api/generate`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: this.model, prompt, stream: false, think: false, format: responseSchema, options: { temperature: 0 } }) });
    if (!response.ok) throw new Error(`Ollama HTTP ${response.status}`); const payload = await response.json(); return parseOllamaJson(payload.response);
  }
}
